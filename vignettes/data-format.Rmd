...
d <- rbind(
  data.frame(
    granularity_time = "event_covid19_norway_2020_02_21_to_9999_01_01",
    definition = "Covid-19 outbreak in Norway (on-going)."
  ),
  data.frame(
    granularity_time = "event_covid19_norway_vaccination_2020_12_02_to_9999_01_01",
...
Locations can be obtained from [spldata](/spldata). Valid locations (and location types) are available in `spldata::norway_locations_names()`.  Uncommon/internal use is demarcated by a line through the text.
...
d <- rbind(
  ...
  data.frame(
    ...
    accepted_values = paste0("\"",paste0(unique(spldata::norway_locations_names()$granularity_geo), collapse="\", \""),"\""),
  ...
  data.frame(
    variable = "age",
    accepted_values = gt::html("\"000\", \"001\", \"000-004\", \"065p\", \"total\", NA_character_, ... (<a href='#age'>Age</a>)"),
    definition = "Age in years"
  ),
  data.frame(
    variable = "sex",
    accepted_values = gt::html("\"male\", \"female\", \"total\", NA_character_ (<a href='#sex'>Sex</a>)"),
    definition = "Sex"
  ),
  data.frame(
    variable = "isoyear",
...
d <- rbind(
  ...
  data.frame(
    x = "",
    examples = "id/tag",
    definition = "Used when data is in long format, to indicate an id variable. Frequently combined with descriptions of 'outcome', 'exposure', 'model'. id is used for numeric variables. tag is used for character variables."
  ),
  data.frame(
    x = "",
    examples = "n",
...
```{r}
d <- spltidy::test_data_generator()
spltidy::set_splfmt_rts_v1(d)
...
# Smart assignment fails when multiple time variables are set
d[10,c("isoyear","isoyearweek") := .(2021,"2021-01")]
d

# Smart assignment of geo variables
...
# removing the class splfmt_rts_v1_mini (this is uncommon)
d[, .(deaths_n = sum(deaths_n), location_code = "norge"), keyby=.(granularity_time)] %>%
  print()

# Collapsing to different levels, and removing the class splfmt_rts_v1_mini because
# it is going to be used in new output/analyses
d[, .(deaths_n = sum(deaths_n), location_code = "norge"), keyby=.(granularity_time)] %>%
  spltidy::remove_class_splfmt_rts_v1_mini() %>%
  print()
