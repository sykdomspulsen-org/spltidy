...
gt::gt(d) %>%
  ...
  gt::tab_footnote(
    footnote = gt::md("**location_code**: Used a) **inside datasets** and b) in data **file names** for transfer of data/results between analytic systems."),
  ...
  gt::tab_footnote(
    footnote = gt::md("**location_name_description_nb**: Used (frequently) **inside results** (figures, tables, documents)."),
    locations = gt::cells_column_labels(
      columns = location_name_description_nb
    )
  ) %>%
  gt::tab_footnote(
    footnote = gt::md("**location_name_file_nb_utf**: Used (frequently) in the **file names** for results (figures, tables, documents)."),
    locations = gt::cells_column_labels(
      columns = location_name_file_nb_utf
    )
  ) %>%
  gt::tab_footnote(
    footnote = gt::md("**location_name_file_nb_ascii**: Used (rarely) in the **file names** for results (figures, tables, documents). Used if file systems have problems with the Norwegian letters æøå."),
    locations = gt::cells_column_labels(
...
```{r echo=FALSE, results='asis'}
d <- spldata::norway_locations_names()[,.(
  location_order = paste0(location_order," / ",.N),
  granularity_geo,
...
gt::gt(d) %>%
  ...
  gt::tab_header(title = "Reference of valid `location_code`s and `location_name_description_nb`s") %>%
  gt::cols_label(
    location_order = "#"
  ) 
```
