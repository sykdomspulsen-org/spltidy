...
Locations can be obtained from [spldata](/spldata). Valid locations are available in `spldata::norway_locations_names()`.
...
gt::gt(d) %>%
  ...
  gt::tab_header(title = "Valid locations in the splverse format") %>%
  gt::cols_label(
  ...
  gt::tab_footnote(
    footnote = gt::md("Used a) **inside datasets** and b) in data **file names** for transfer of data/results between analytic systems."),
    locations = gt::cells_column_labels(
      columns = location_code
    )
  ) %>%
  gt::tab_footnote(
    footnote = gt::md("Used (rarely) **inside results** (figures, tables, documents). Can be confusing as some names are duplicated."),
    locations = gt::cells_column_labels(
  ...
  gt::tab_footnote(
    footnote = gt::md("Used (frequently) **inside results** (figures, tables, documents)."),
    locations = gt::cells_column_labels(
      columns = location_name_description_nb
    )
  ) %>%
  gt::tab_footnote(
    footnote = gt::md("Used (frequently) in the **file names** for results (figures, tables, documents)."),
    locations = gt::cells_column_labels(
      columns = location_name_file_nb_utf
    )
  ) %>%
  gt::tab_footnote(
    footnote = gt::md("Used (rarely) in the **file names** for results (figures, tables, documents). Used if file systems have problems with the Norwegian letters æøå."),
    locations = gt::cells_column_labels(
